...
save_window_size (GtkWidget *dialog, XfceSettingsEditorBox *settings_editor)
{
	...
	if ((state & (GDK_WINDOW_STATE_MAXIMIZED | GDK_WINDOW_STATE_FULLSCREEN)) == 0)
	{
		...
		blconf_channel_set_int (channel, "/last/window-width", width),
			blconf_channel_set_int (channel, "/last/window-height", height);
		blconf_channel_set_int (channel, "/last/paned-position", paned_pos);
...
main(gint argc, gchar **argv)
{
	...
	channel = blconf_channel_new ("blade-settings-editor");

	settings_editor = xfce_settings_editor_box_new (
		blconf_channel_get_int (channel, "/last/paned-position", 180));
	...
	if (G_UNLIKELY (opt_socket_id == 0))
    {
		...
		gtk_window_set_default_size (GTK_WINDOW (dialog),
          blconf_channel_get_int (channel, "/last/window-width", 640),
          blconf_channel_get_int (channel, "/last/window-height", 500));
