...
#include "blade-settings-editor-box.h"
#include "blade-settings-prop-dialog.h"
#include "blade-settings-cell-renderer.h"
...
xfce_settings_editor_box_load_channels (XfceSettingsEditorBox *self)
{
    ...
    channel_names = blconf_list_channels ();
        ...
        if (!found_parent)
        {
            ...
            else
            {
                ...
                gtk_tree_store_set (GTK_TREE_STORE (model), &child_iter,
                                    ...
                                    PROP_COLUMN_LOCKED, blconf_channel_is_property_locked (self->props_channel, property),
...
static void
xfce_settings_editor_box_property_changed (BlconfChannel            *channel,
            ...
            if (gtk_tree_model_get_iter (model, &child_iter, context->path))
            {
                ...
                else
                {
                    ...
                    while (has_parent)
                    {
                        ...
                        /* nope it points to a real blconf property */
                        if (!empty_prop)
                            break;
										  ...
										  BlconfChannel            *channel)
{
    ...
    self->props_channel = (BlconfChannel *) g_object_ref (G_OBJECT (channel));

    props = blconf_channel_get_properties (channel, NULL);
										  ...
										  XfceSettingsEditorBox *self)
{
    ...
    if (gtk_tree_selection_get_selected (selection, NULL, &iter))
    {
        ...
        channel = blconf_channel_new (g_value_get_string (&value));

        locked = blconf_channel_is_property_locked (channel, "/");
...
xfce_settings_editor_box_channel_reset (XfceSettingsEditorBox *self)
{
                             ...
                             channel_name))
    {
        ...
        blconf_channel_reset_property (self->props_channel, "/", TRUE);

        /* check if the channel still exists (channel reset, not remove) */
        channels = blconf_list_channels ();
...
static void
xfce_settings_editor_box_channel_monitor_changed (BlconfChannel *channel,
												  const gchar   *property,
												  const GValue  *value,
												  GtkWidget     *window)
{
...
xfce_settings_editor_box_channel_monitor (XfceSettingsEditorBox *self)
{
    ...
    xfce_titled_dialog_set_subtitle (XFCE_TITLED_DIALOG (window),
        _("Watch an Blconf channel for property changes"));
										...
										XfceSettingsEditorBox    *self)
{
        ...
        && gtk_tree_model_get_iter (model, &iter, path))
    {
        ...
        {
            if (!blconf_channel_is_property_locked (self->props_channel, property))
                blconf_channel_set_property (self->props_channel, property, new_value);
            g_free (property);
        }
    }
    gtk_tree_path_free (path);
											...
											XfceSettingsEditorBox    *self)
{
    ...
    g_return_if_fail (self->props_channel == NULL
                      || BLCONF_IS_CHANNEL (self->props_channel));
        ...
        && gtk_widget_get_sensitive (self->button_new))
    {
        ...
        can_edit = !blconf_channel_is_property_locked (self->props_channel, property);
...
xfce_settings_editor_box_property_reset (XfceSettingsEditorBox *self)
{
    ...
    {
        blconf_channel_reset_property (self->props_channel, property, FALSE);
    }
