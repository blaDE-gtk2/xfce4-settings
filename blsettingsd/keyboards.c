...
static void xfce_keyboards_helper_channel_property_changed  (BlconfChannel            *channel,
                                                             const gchar              *property_name,
                                                             const GValue             *value,
                                                             XfceKeyboardsHelper      *helper);
static void xfce_keyboards_helper_restore_numlock_state     (BlconfChannel            *channel);
static void xfce_keyboards_helper_save_numlock_state        (BlconfChannel            *channel);
...
xfce_keyboards_helper_init (XfceKeyboardsHelper *helper)
{
    ...
    {
        blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "initialized xkb %d.%d", marjor_ver, minor_ver);

        /* open the channel */
        helper->channel = blconf_channel_get ("keyboards");
...
xfce_keyboards_helper_set_auto_repeat_mode (XfceKeyboardsHelper *helper)
{
    ...
    /* load setting */
    repeat = blconf_channel_get_bool (helper->channel, "/Default/KeyRepeat", TRUE);
    ...
    blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "set auto repeat %s", repeat ? "on" : "off");
}
...
xfce_keyboards_helper_set_repeat_rate (XfceKeyboardsHelper *helper)
{
    ...
    /* load settings */
    delay = blconf_channel_get_int (helper->channel, "/Default/KeyRepeat/Delay", 500);
    rate = blconf_channel_get_int (helper->channel, "/Default/KeyRepeat/Rate", 20);
    ...
    if (G_LIKELY (xkb))
    {
        ...
        blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "set key repeat (delay=%d, rate=%d)",
...
static void
xfce_keyboards_helper_channel_property_changed (BlconfChannel      *channel,
                                               const gchar         *property_name,
                                               const GValue        *value,
                                               XfceKeyboardsHelper *helper)
{
...
static void
xfce_keyboards_helper_restore_numlock_state (BlconfChannel *channel)
{
    unsigned int  numlock_mask;
    Display      *dpy;
    gboolean      state;

    if (blconf_channel_has_property (channel, "/Default/Numlock")
        && blconf_channel_get_bool (channel, "/Default/RestoreNumlock", TRUE))
    {
        state = blconf_channel_get_bool (channel, "/Default/Numlock", FALSE);
        ...
        blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "set numlock %s", state ? "on" : "off");
    }
    else
    {
        blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "don't set numlock");
    }
}
...
static void
xfce_keyboards_helper_save_numlock_state (BlconfChannel *channel)
{
    Display *dpy;
    Bool     numlock_state;
    Atom     numlock;
    ...
    blsettings_dbg (XFSD_DEBUG_KEYBOARDS, "save numlock %s", numlock_state ? "on" : "off");

    blconf_channel_set_bool (channel, "/Default/Numlock", numlock_state);
}
