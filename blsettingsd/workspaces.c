...
static void             xfce_workspaces_helper_prop_changed (BlconfChannel        *channel,
...
xfce_workspaces_helper_init (XfceWorkspacesHelper *helper)
{
    ...
    helper->channel = blconf_channel_get(WORKSPACES_CHANNEL);
                                    ...
                                    gpointer    user_data)
{
    ...
    if (xevent->type == PropertyNotify)
    {
        if (xevent->xproperty.atom == atom_net_number_of_desktops)
        {
            ...
            blsettings_dbg (XFSD_DEBUG_WORKSPACES, "number of desktops changed");
        }
        else if (xevent->xproperty.atom == atom_net_desktop_names)
        {
                    ...
                    && timestamp.tv_usec > helper->timestamp.tv_usec))
            {
                 ...
                 * not update blconf) the name of a desktop, store the
                 * new names in blconf if different*/
                xfce_workspaces_helper_save_names (helper);

                blsettings_dbg (XFSD_DEBUG_WORKSPACES, "someone else changed the desktop names");
            }
        }
    }
#endif
...
xfce_workspaces_helper_set_names_real (XfceWorkspacesHelper *helper)
{
    ...
    /* check if there are enough names in blconf, else we save new
     * names first and set the names the next time property-changed is
     * triggered on the channel */
    names = blconf_channel_get_arrayv (helper->channel, WORKSPACE_NAMES_PROP);
    if (names != NULL && names->len >= n_workspaces)
    {
        /* store this in blconf (for no really good reason actually) */
        blconf_channel_set_int (helper->channel, WORKSPACE_COUNT_PROP, n_workspaces);
        ...
        for (i = 0; i < names->len && i < n_workspaces; i++)
        {
            ...
            {
                /* value in blconf isn't a string, so make a default one */
        ...
        blsettings_dbg (XFSD_DEBUG_WORKSPACES, "%d desktop names set from blconf", i);
    ...
    else
    {
        ...
        /* store new array in blconf */
        if (!blconf_channel_set_arrayv (helper->channel, WORKSPACE_NAMES_PROP, names))
             g_critical ("Failed to save blconf property %s", WORKSPACE_NAMES_PROP);

        blsettings_dbg (XFSD_DEBUG_WORKSPACES, "extended names in blconf, waiting for property-change");

        blconf_array_free (existing_names);
    }
...
{
    GPtrArray   *blconf_names, *new_names;
    GValue      *val_b;
    ...
    blconf_names = blconf_channel_get_arrayv (helper->channel, WORKSPACE_NAMES_PROP);

    if (blconf_names == NULL
       || blconf_names->len < new_names->len)
    {
        /* store the new names in blconf, should not append often because
         * we probably saved (and set) a name when the workspace count
         * was changed */
        blconf_channel_set_arrayv (helper->channel, WORKSPACE_NAMES_PROP, new_names);

        blsettings_dbg (XFSD_DEBUG_WORKSPACES, "storing %d names in blconf", new_names->len);
    }
    else if (blconf_names != NULL
             && blconf_names->len >= new_names->len)
    {
        /* update the new names in the blconf array */
        for (i = 0; i < new_names->len; i++)
        {
             name_a = g_value_get_string (g_ptr_array_index (new_names, i));

             val_b = g_ptr_array_index (blconf_names, i);
             name_b = g_value_get_string (val_b);

             if (g_strcmp0 (name_a, name_b) != 0)
             {
                 /* set the old blconf name to the new name */
                 g_value_unset (val_b);
        ...
        if (save_array)
        {
            blconf_channel_set_arrayv (helper->channel, WORKSPACE_NAMES_PROP, blconf_names);

            blsettings_dbg (XFSD_DEBUG_WORKSPACES, "merged %d blconf and %d desktop names",
                            blconf_names->len, new_names->len);
        }
    }

    if (blconf_names != NULL)
        blconf_array_free (blconf_names);
    blconf_array_free (new_names);
}



static void
xfce_workspaces_helper_prop_changed (BlconfChannel        *channel,
                                     const gchar          *property,
                                     const GValue         *value,
                                     XfceWorkspacesHelper *helper)
{
