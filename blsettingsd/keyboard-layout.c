...
static void xfce_keyboard_layout_helper_channel_property_changed  (BlconfChannel                 *channel,
...
xfce_keyboard_layout_helper_init (XfceKeyboardLayoutHelper *helper)
{
    ...
    /* open the channel */
    helper->channel = blconf_channel_get ("keyboard-layout");

    helper->xkb_disable_settings = blconf_channel_get_bool (helper->channel, "/Default/XkbDisable", TRUE);
...
xfce_keyboard_layout_helper_process_xmodmap (void)
{
    ...
    if (g_file_test (xmodmap_path, G_FILE_TEST_EXISTS))
    {
        ...
        blsettings_dbg (XFSD_DEBUG_KEYBOARD_LAYOUT, "spawning \"%s\"", xmodmap_command);
...
xfce_keyboard_layout_helper_set_model (XfceKeyboardLayoutHelper *helper)
{
    ...
    {
        xkbmodel = blconf_channel_get_string (helper->channel, "/Default/XkbModel", NULL);
        ...
        if (g_strcmp0 (helper->config->model, xkbmodel) != 0)
        {
            ...
            blsettings_dbg (XFSD_DEBUG_KEYBOARD_LAYOUT, "set model to \"%s\"", xkbmodel);
        }
        else
        {
            g_free (xkbmodel);
...
xfce_keyboard_layout_helper_set (XfceKeyboardLayoutHelper *helper,
                                 const gchar *blconf_option_name,
                                 gchar ***xkl_config_option,
                                 const gchar *debug_name)
{
    gchar *blconf_values, *xkl_values;
    gchar **values;

    if (!helper->xkb_disable_settings)
    {
        blconf_values  = g_strjoinv (",", *xkl_config_option);
        xkl_values  = blconf_channel_get_string (helper->channel,
                                                 blconf_option_name, blconf_values);

        if (g_strcmp0 (blconf_values, xkl_values) != 0)
        {
            ...
            *xkl_config_option = values;
            ...
            blsettings_dbg (XFSD_DEBUG_KEYBOARD_LAYOUT, "set %s to \"%s\"", debug_name, xkl_values);
        }

        g_free (blconf_values);
        g_free (xkl_values);
    }
}
                                        ...
                                        const gchar *blconf_option_name)
{
    if (!helper->xkb_disable_settings)
    {
        ...
        option_value = blconf_channel_get_string (helper->channel, blconf_option_name,
                                                  xkl_option_value);
        if (g_strcmp0 (option_value, xkl_option_value) != 0)
        {
            ...
            blsettings_dbg (XFSD_DEBUG_KEYBOARD_LAYOUT, "set %s to \"%s\"",
...
static void
xfce_keyboard_layout_helper_channel_property_changed (BlconfChannel      *channel,
                                               const gchar               *property_name,
                                               const GValue              *value,
                                               XfceKeyboardLayoutHelper  *helper)
{
                                       ...
                                       XfceKeyboardLayoutHelper *helper)
{
    if (!helper->xkb_disable_settings)
    {
        ...
        blsettings_dbg (XFSD_DEBUG_KEYBOARD_LAYOUT,
                        "New keyboard detected; restoring XKB settings.");

        xkl_config_rec_reset (helper->config);
        xkl_config_rec_get_from_server (helper->config, helper->engine);

        blconf_model = blconf_channel_get_string (helper->channel, "/Default/XkbModel", NULL);
        if (blconf_model && *blconf_model &&
            g_strcmp0 (blconf_model, helper->config->model) != 0 &&
