...
static void xfce_decorations_helper_channel_property_changed  (BlconfChannel            *channel,
                                        ...
                                        const gchar *value)
{
    ...
    blconf_channel_set_string (helper->xsettings_channel,
                             "/Gtk/DecorationLayout", gtk_decoration_layout);
    g_free (gtk_decoration_layout);
}

static void
xfce_decorations_helper_channel_property_changed (BlconfChannel         *channel,
                                                  const gchar           *property_name,
                                                  const GValue          *value,
                                                  XfceDecorationsHelper *helper)
{
...
xfce_decorations_helper_init (XfceDecorationsHelper *helper)
{
    ...
    helper->wm_channel = blconf_channel_get ("xfwm4");
    helper->xsettings_channel = blconf_channel_get ("xsettings");

    layout = blconf_channel_get_string  (helper->wm_channel,
